...
def multivar(q, l):
    """ Convert a query ``q`` to multivalue query based on list ``l``

    The number of arguments in the query will reflect the length of the list
    ``l``.

    :param q:   query
    :param l:   list
    """
    return q.replace('??', ', '.join('?' * len(l)))
